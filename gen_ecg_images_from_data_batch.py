...
def get_parser():
    ...
    parser.add_argument('--store_config',action="store_true",default=True)
...
def run(args):
        ...
        if not os.path.exists(original_output_dir + "/log.xlsx"):
            workbook = openpyxl.Workbook()
            workbook.save(original_output_dir + "/log.xlsx")
            wb = load_workbook(original_output_dir + "/log.xlsx")
            ws = wb['Sheet']
            ws.cell(1,1).value = "name"
            ws.cell(1,2).value = "dc"
            ws.cell(1,3).value = "num_col"
            ws.cell(1,4).value = "header"
            ws.cell(1,5).value = "leader_name"
            wb.save(original_output_dir + "/log.xlsx")
        i = 0
        n = 0
