...
def train_models(data_folder, model_folder, verbose):
    ...
    # generator.
    try:
        digitization_model = train_digitization_model(records, data_folder, model_folder)
    except Exception as e:
        print(f'An error occurred while training the digitization model: {e}')
    ...
    try:
        classification_model = train_classification_model(records, data_folder, model_folder)
    except Exception as e:
        print(f'An error occurred while training the classification model: {e}')
        classification_model = None
    end_time = datetime.now()
    print(f"运行时间: {(end_time - start_time).total_seconds()} 秒")
...
def load_models(model_folder, verbose):
    digitization_filename = os.path.join(model_folder, 'best_digital_model.pth')
    if not os.path.exists(digitization_filename):
    ...
    # classify
    classification_filename = os.path.join(model_folder, 'classification_new_train.pth')
    if not os.path.exists(classification_filename):
        classification_filename = os.path.join(model_folder, 'train_v5_classification.pth')
        # print("use pretrain classifiy model without thresholds!!!")
    ...
    thresholds = list()
    if classification_filename!=os.path.join(model_folder, 'train_v5_classification.pth'):
        thresholds = checkpoints['thresholds'] # list
    
    classification_model = {}
    classification_model["classify"] = saved_model_classify
    classification_model["threshold"] = thresholds # 为空就是空list
...
def run_models(record, digitization_model, classification_model, verbose):
    ...
    # 第7版本
    thresholds_list = classification_model["threshold"]
    if not thresholds_list:
        thresholds_list = [0.09999999999999999, 0.02, 0.06999999999999999, 0.21000000000000002, 0.12, 0.02, 0.01, 0.02, 0.09999999999999999, 0.13, 0.02]
        # print("use pretrain thred...")
    # extract 
    with torch.no_grad():
        # 用v3 的train，用v3
        # features = extract_for_classify(classification_model["model_folder"],image_classify) # (1,3,299,299)
        features = extract_features1(array_3d) # 用v5.2
        features = features.to(device)
...
def train_digitization_model(records, dir_data, dir_model):
    # split records into train and val
    train_records, val_records = train_test_split(records, test_size=0.2, random_state=42)
    # create dataset
    train_dataset = ECGImageDataset(train_records, dir_data, augmentation=get_training_augmentation())
    val_dataset = ECGImageDataset(val_records, dir_data, augmentation=get_validation_augmentation())
    # 
    epochs = 30
    learning_rate = 1e-3
    early_stop = 5
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    model = UNetModel()
    model.load_state_dict(torch.load(f'{dir_model}/digitization_model_v5.2.pth'))
    model.to(device)
    loss_fn = DiceLoss()
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_dataloader):
            inputs, labels, signal, info = data # inputs are image, labels are masks
            if any(info['bad_flag']):
                print(f'bad flag: {info["record"]}')
                continue
            ...
            with torch.no_grad():
                running_metric += metric(outputs, labels)
        ...
        if last_loss == 0 and last_metric == 0:
            last_loss = running_loss / (i + 1)
            last_metric = running_metric / (i + 1)
        avg_loss = last_loss
        avg_metric = last_metric
        ...
        with torch.no_grad():
            for i, vdata in enumerate(val_dataloader):
                vinputs, vlabels, signal, info = vdata
                if any(info['bad_flag']):
                    print(f'bad flag: {info["record"]}')
                    continue
                ...
                metric_value = metric(voutputs, vlabels)
                running_vmetric += metric_value
        ...
        if avg_vloss < best_vloss:
            ...
            model_path = f'{dir_model}/best_digital_model.pth'
            torch.save(model.state_dict(), model_path)
            # model.save_pretrained(model_path)

        if early_stop > 0 and early_stopper.early_stop(avg_vloss):             
            break
        epoch_number += 1
        lr_scheduler.step()
...
def extract_features_train(model_folder_q,record):
    """
        record : 
        model_folder
        return :3,512,512
    ...
    # 去背景
    model = UNetModel()
    model_path = os.path.join(model_folder_q, 'best_digital_model.pth')
    if not os.path.exists(model_path):
        model_path = os.path.join(model_folder_q, 'digitization_model_v5.pth')
        # print("use pretrain dig_v5 model to remove the background of image")
    ...
    pixel_array = post_processing(pixel_array)
    pixel_array = 255 - pixel_array
    array_3d = np.stack([pixel_array] * 3, axis=0) # (3,512,512)

    # print(pixel_array.shape) # (512,512)
    # im = Image.fromarray(pixel_array)
    return array_3d
...
def train_classification_model(records,dir_data,model_folder):
    """
        records :
        dir_data :
        model_folder:
    ...
    train_records, val_records = train_test_split(records, test_size=0.2, random_state=42)
    # create dataset
    num_records = len(train_records) # train len
    if num_records == 0:
        raise FileNotFoundError('No data was provided.')
    
    classification_features = list()
    classification_labels = list() # label
    
    for i in range(num_records):
        
        width = len(str(num_records))
        print(f'- {i+1:>{width}}/{num_records}: {train_records[i]}...')
        record = os.path.join(dir_data, train_records[i]) # record就是路径名称
        # Extract the features from the image, but only if the image has one or more dx classes.
       
        labels = load_labels(record) # load dx label from .hea
        features = extract_features_train(model_folder,record) # 3,512,512
        # Some images may not be labeled...
        if any(label for label in labels):
            classification_features.append(features)
            classification_labels.append(labels)
    ...
    # val
    num_records_val = len(val_records) # train len
    classification_features_val = list()
    classification_labels_val = list() # label
    for i in range(num_records_val):
        width = len(str(num_records_val))
        print(f'- {i+1:>{width}}/{num_records_val}: {val_records[i]}...')
        record = os.path.join(dir_data, val_records[i]) # record就是路径名称
        # Extract the features from the image, but only if the image has one or more dx classes.
        ...
        # Some images may not be labeled...
        if any(label for label in labels):
            classification_features_val.append(features)
            classification_labels_val.append(labels)
    
    label_val = encode_labels(classification_labels_val, label_to_index)
    ...
    batch_size = 8
    dataset = MyDataset(classification_features, encoded_data_labels,transform=transform)
    dataloader = DataLoader(dataset, batch_size=batch_size)
    dataset_val = MyDataset(classification_features_val, label_val,transform=transform)
    dataloader_val = DataLoader(dataset_val, batch_size=batch_size)
    # extract classifiy feature
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    for _ in tqdm(range(n_epoch), desc="epoch", leave=False):
        ...
        # val
        model2.eval()
        prog_iter_test = tqdm(dataloader_val, desc="Valing", leave=False)
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(prog_iter_test):
                ...
                pred = torch.sigmoid(model2(input_x))
                all_score.append(pred.cpu().data.numpy())
                predicted_labels = (pred >= 0.5).float() # 大于设为1，小于设为0
                all_pred_val.append(predicted_labels.cpu().data.numpy())
        pred_val = np.concatenate(all_pred_val)
        score_val = np.concatenate(all_score)
        true_val = np.concatenate(all_y_val)
        thresholds_list = find_optimal_thresholds(true_val,score_val)
    ...
    }, os.path.join(model_folder, 'classification_new_train.pth')) # 保存路径
