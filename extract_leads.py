...
from ecg_plot_new import ecg_plot_new
import wfdb
...
# Run script.
def get_paper_ecg(style_path,input_file,header_file,output_directory, seed, add_dc_pulse,add_bw,show_grid,add_print, start_index = -1, store_configs=False, store_text_bbox=True,key='val',resolution=100,units='inches',papersize='',add_lead_names=True,pad_inches=1,template_file=os.path.join('TemplateFiles','TextFile1.txt'),font_type=os.path.join('Fonts','Times_New_Roman.ttf'),standard_colours=5,full_mode='II',bbox = False,columns=-1,ful_mode=0):
            ...
            for key in record_dict:
                ...
                if(len(record_dict[key][start:])<int(rate*abs_lead_step)):
                    ...
                    if(key!='full'+full_mode):
                        ...
                        if len(record_dict[key]) == 5000:
                            if columns == 2 and key in ['aVF', 'aVL', 'aVR', 'III', "II", 'I']:
                                shilftedStart = 0
                                end = 2500
                            if columns == 2 and key in ['V6', 'V5', 'V4', 'V3', 'V2', 'V1']:
                                shilftedStart = 2500
                                end = 5000
                        else:
                            if columns == 2 and key in ['aVF', 'aVL', 'aVR', 'III', "II", 'I']:
